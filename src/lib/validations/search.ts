...
export const searchOpportunitiesSchema = z.object({
  ...
  departments: z.array(z.string().uuid()).max(20, 'Maximum 20 departments allowed').optional(),

  locationType: z.enum(['ON_CAMPUS', 'OFF_CAMPUS', 'HYBRID', 'REMOTE']).optional(),

  commitmentType: z.enum(['SHORT_TERM', 'MEDIUM_TERM', 'LONG_TERM', 'ONGOING']).optional(),

  availableSpots: z.boolean().optional(), // Only show opportunities with available spots
...
export const searchUsersSchema = z.object({
  ...
  programType: z.enum(['bachelor', 'master', 'phd']).optional(),

  isActive: z.boolean().optional(),

  // Sorting
  sortBy: z
    .enum(['NAME_ASC', 'NAME_DESC', 'EMAIL_ASC', 'EMAIL_DESC', 'CREATED_AT_DESC', 'CREATED_AT_ASC'])
    .default('NAME_ASC'),
...
export const filterSavedOpportunitiesSchema = z.object({
  ...
  status: z.enum(['PUBLISHED', 'IN_PROGRESS', 'COMPLETED']).optional(),

  sortBy: z
    .enum(['SAVED_DATE_DESC', 'SAVED_DATE_ASC', 'TITLE_ASC', 'TITLE_DESC', 'DEADLINE_SOON'])
